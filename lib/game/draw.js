...
export function createDrawFunctions(ctx, canvas, ship, state) {
  ...
  function drawMeteorStormBanner() {
    ...
    // Safe zone: highlight the safe third with a green tint + label
    const zoneW = canvas.width / 3;
    const zoneX = state.meteorStormSafePos * zoneW;
    const zoneH = 90;
    const zy = canvas.height - zoneH;
    ctx.save();
    const zFill = ctx.createLinearGradient(0, zy, 0, canvas.height);
    zFill.addColorStop(0, `rgba(0,255,100,0)`);
    zFill.addColorStop(1, `rgba(0,255,100,${baseAlpha * 0.2})`);
    ctx.fillStyle = zFill;
    ctx.fillRect(zoneX, zy, zoneW, zoneH);
    ctx.strokeStyle = `rgba(0,255,100,${baseAlpha * 0.6})`;
    ctx.lineWidth = 2;
    ctx.strokeRect(zoneX + 1, zy, zoneW - 2, zoneH);
    const arrowBounce = Math.sin(Date.now() * 0.008) * 5;
    ctx.globalAlpha = baseAlpha * (0.75 + 0.25 * Math.sin(Date.now() * 0.008));
    ctx.font = "bold 14px Arial";
    ctx.textAlign = "center";
    ctx.fillStyle = "#00FF64";
    ctx.shadowColor = "#00FF64";
    ctx.shadowBlur = 14;
    ctx.fillText("✓ SAFE ZONE", zoneX + zoneW / 2, canvas.height - 8);
    ctx.font = "bold 20px Arial";
    ctx.fillText("▼", zoneX + zoneW / 2, zy - 4 + arrowBounce);
    ctx.shadowBlur = 0;
    ctx.restore();
  }
