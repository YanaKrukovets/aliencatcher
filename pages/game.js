...
const GAME_URL = "https://www.alifallx.com/game";

function ShareButtons({ score, level, complete }) {
  const [linkedInHint, setLinkedInHint] = useState(false);

  const text = complete
    ? `I rescued the Last Egg in AlifallX! 🥚 Score: ${score}, Level: ${level}. Can you do it? ${GAME_URL}`
    : `I caught ${score} aliens and reached Level ${level} in AlifallX! 👽 Can you beat my score? ${GAME_URL}`;

  const encodedText = encodeURIComponent(text);
  const encodedUrl = encodeURIComponent(GAME_URL);

  function openShare(href) {
    window.open(href, "share", "width=600,height=460,resizable,scrollbars=yes,status=1");
  }

  function handleLinkedIn() {
    navigator.clipboard.writeText(text).catch(() => {});
    setLinkedInHint(true);
    setTimeout(() => setLinkedInHint(false), 4000);
  ...
  const platforms = [
    ...
    },
    {
      label: "Facebook",
      color: "#1877F2",
      onClick: () => openShare(`https://www.facebook.com/sharer/sharer.php?u=${encodedUrl}&quote=${encodedText}`),
      icon: (
        <svg width="15" height="15" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
  ...
  return (
    <div className="share-row">
      <div className="share-label">Share your result</div>
      <div className="share-buttons">
        {platforms.map(({ label, color, onClick, icon }) => (
          <button
            key={label}
            className="share-btn"
      ...
      {linkedInHint && (
        <div className="share-linkedin-hint">Text copied — paste it into your LinkedIn post</div>
      )}
    </div>
  );
}
        ...
        <div className="game-canvas-area">
          ...
          {isGameOver && (
            <div className="game-screen game-screen--dark" style={{ gap: 28 }}>
              ...
              <ShareButtons score={score} level={level} complete={false} />
          ...
          {isMissionComplete && (
            <div className="game-screen game-screen--green" style={{ gap: 26 }}>
              ...
              <ShareButtons score={score} level={level} complete={true} />
              <button className="play-btn" aria-label="Play again" onClick={handleRestart}>
