...
## Project Overview
Space arcade game built with **Next.js 15 + Tailwind CSS**. Players control a spaceship on an HTML5 canvas, dodge rocks, and catch/destroy aliens. The site also has a landing page with About, Contact, and game info sections.
...
| `components/AlienIcon.js` | SVG alien icon component (used in landing page) |
| `components/BackToTopButton.js` | Scroll-to-top button |
| `components/FAQ.js` | FAQ accordion section |
| `components/Starfield.js` | Animated star background for landing page |
| `pages/_document.jsx` | Custom Document (HTML shell) |
...
- Level progression: level 1 while score < 3, then iterative cost-based formula — L2 costs 5 pts, each level costs more as levels increase (5 pts/level up to L10, 7 pts L10–15, 10 pts L15–20, 15 pts L20+). Each level increases alien/rock speed (`+0.2/level`), tightens spawn intervals (min 55ms at high levels), and cycles color palettes (`LEVEL_PALETTES`/`LEVEL_COLORS`).
